...
function StockCard({ stock, onNavigate }) {
  const baseSymbol = stock.ticker?.replace('.IS', '') || '';
  
  return (
    <div
      className="glass-card glass-card-hover p-4 cursor-pointer group transition-all duration-200 hover:scale-[1.01]"
      onClick={() => onNavigate(baseSymbol)}
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2.5">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-purple-500/20 to-cyan-500/20 border border-purple-500/20 flex items-center justify-center text-xs font-bold text-purple-300">
          ...
          <div>
            ...
            <div className="text-[10px] text-slate-500 truncate max-w-[140px]">{stock.name?.replace(' (BIST Hisse Senedi)', '').replace(' (Otomatik Eklendi)', '').replace(' (Otomatik Keşif)', '')}</div>
          </div>
        </div>
        {stock.lastPrice && (
          <div className="text-right">
            <div className="text-sm font-mono font-bold text-white">₺{stock.lastPrice?.toLocaleString('tr-TR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center justify-between text-[10px]">
        ...
        <div className="flex items-center gap-1 text-slate-500">
          <Eye size={10} />
          <span>Detay</span>
          <ArrowUpRight size={10} className="text-purple-400 opacity-0 group-hover:opacity-100 transition-opacity" />
        </div>
      </div>
      ...
      <td className="py-3 px-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[10px] text-slate-500 truncate max-w-[160px]">{stock.name?.replace(' (BIST Hisse Senedi)', '').replace(' (Otomatik Eklendi)', '').replace(' (Otomatik Keşif)', '')}</div>
          </div>
        </div>
      </td>
      ...
      {stats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={i} className={`glass-card p-3 bg-gradient-to-br ${item.color}`}>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          {displayStocks.map(stock => (
            <StockCard key={stock.ticker} stock={stock} onNavigate={navigateToStock} />
          ))}
        </div>
      ) : (
        /* Table View */
            ...
            <table className="w-full">
              <thead>
                <tr className="text-[10px] text-slate-500 uppercase tracking-wider border-b border-white/5">
                  ...
                  <th className="py-3 px-3 hidden md:table-cell cursor-pointer hover:text-slate-300" onClick={() => handleSort('sector')}>Sektör</th>
